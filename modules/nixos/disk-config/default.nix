...
#
# Snowfall auto-discovers this module and includes it on every NixOS
# host, so we gate the actual disko declaration behind
# `local.lunaDisk.enable` (default `false`) to keep the disk spec
# off of any host that isn't luna. Luna's system config explicitly
# opts in via `local.lunaDisk.enable = true;`.
#
...
in
{
  options.local.lunaDisk = {
    enable = lib.mkEnableOption ''
      luna's disko partition layout. Default off — only luna opts in.
      Auto-discovered by snowfall from `modules/nixos/disk-config/`
      and therefore imported on every NixOS host; gating the disko
      spec prevents it from wiping partitions on unrelated machines.
    '';
  };
