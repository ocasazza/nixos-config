# Gemini CLI configuration for Home Manager.
{
  config,
  lib,
  pkgs,
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    home.sessionVariables = {
      GEMINI_TELEMETRY_ENABLED = if cfg.telemetry.enable then "true" else "false";
      GEMINI_SANDBOX = if cfg.sandbox.enable then cfg.sandbox.driver else "false";
      SEATBELT_PROFILE = cfg.seatbeltProfile;
    };
    ...
    home.file.".gemini/settings.json".source = (pkgs.formats.json { }).generate "gemini-settings.json" (
      lib.recursiveUpdate {
        ...
        tools = lib.optionalAttrs (cfg.seatbeltProfile == "docker") {
          sandbox = "${config.home.homeDirectory}/.gemini/sandbox-macos-docker.sb";
        };
      } cfg.extraSettings
    );
