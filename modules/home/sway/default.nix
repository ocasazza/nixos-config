  ...
  options.local.sway = {
    enable = mkOption {
      ...
      description = ''
        ...
        through `home-manager.sharedModules` — never tries to set
        Wayland options on macOS. Mirrors the `local.dock` pattern in
        modules/darwin/dock (`default = stdenv.isDarwin`).
      '';
    };
  };
