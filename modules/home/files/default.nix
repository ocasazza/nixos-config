...
# bspwm/rofi/sxhkd/polybar user-config files for NixOS desktops.
# Snowfall auto-discovers this module and applies it to every HM user,
# but the payload is gated on Linux so the Darwin HMs get a no-op.
#
# Uses HM's canonical `config.home.homeDirectory` / `config.home.username`
# instead of the previous `user.name` module-arg indirection.
let
  ...
  home.file = lib.mkIf pkgs.stdenv.hostPlatform.isLinux {
    ...
    "${xdg_configHome}/sxhkd/sxhkdrc" = {
      text = ''
        ...
        # Program launcher
        super + @space
              rofi -config -no-lazy-grab -show drun -modi drun -theme /home/${username}/.config/rofi/launcher.rasi

        # Terminal emulator
        super + Return
              bspc rule -a Alacritty -o state=floating rectangle=1024x768x0x0 center=true && /etc/profiles/per-user/${username}/bin/alacritty

        # Terminal emulator
        super + ctrl + Return
              /etc/profiles/per-user/${username}/bin/alacritty
        ...
        # Keepass XC
        super + shift + x
              /etc/profiles/per-user/${username}/bin/keepassxc
    ...
    "${xdg_configHome}/polybar/bin/search-nixos-updates.sh" = {
      ...
      text = ''
        ...
        /etc/profiles/per-user/${username}/bin/google-chrome-stable --new-window "https://search.nixos.org/packages"
      '';
    };
